...
const PayApi = {
  ...
  payReady : async function(userIndex, seatIndex, value, amount, point, method, tid, total, kakaoTaxFreeAmount) {
    ...
    return await axios.post(KH_DOMAIN + "/api/reserve/payment", payReadyObj, HEADER);
  },
  // 결제 환불
  payCancel : async function(ticket, refundAmount) {
    return await axios.get(KH_DOMAIN + `/api/reserve/refund/${ticket}/${refundAmount}`, HEADER);
  },
  // 결제 내역 조회
  paySelect : async function(index) {
    return await axios.get(KH_DOMAIN + `/api/reserve/list/payment/${index}`, HEADER);
  },
  payCancelSelect : async function(index) {
    return await axios.get(KH_DOMAIN + `/api/reserve/list/refund-cancel/${index}`, HEADER);
  }
  // 결제 취소 내역
